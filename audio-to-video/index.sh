...
for source in $(ls -1 ./furkies-purrkies/*/files/poetry-*.mp3 | sort -r); do
  ...
  if [ "$execute" = true ] ; then
    ...
    ffmpeg -y -i "${source}" -filter_complex "[0:a]showspectrum=s=1280x760:slide=scroll:mode=separate:color=plasma:scale=5thrt:win_func=sine:orientation=horizontal:legend=true,format=yuv420p,crop=1562:878:0:0[v]" -map "[v]" -map 0:a "${destination}"
  fi;
